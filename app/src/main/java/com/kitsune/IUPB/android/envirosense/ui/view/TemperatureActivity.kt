...
class TemperatureActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        sensorViewModel.sensorDataByMunicipality.observe(this, Observer { groupedData ->
            updateCharts(groupedData)
        })

        // Cargar los datos de temperatura (SensorType: "Temperature")
        sensorViewModel.fetchAndGroupData("Temperature")
    }
    ...
    private fun updateCharts(groupedData: Map<String, List<SensorData>>) {
        ...
        // Configurar BarChart
        val barDataSet = BarDataSet(barEntries, "Total Temperatura por Municipio").apply {
        ...
        // Configurar PieChart
        val pieDataSet = PieDataSet(pieEntries, "Distribución de Temperatura").apply {
            setColors(*colors.toIntArray()) // Convierte la lista de colores a un array de Int
        ...
        val lineDataSet = LineDataSet(lineEntries, "Temperatura por Municipio").apply {
            color = Color.BLUE
            lineWidth = 2f
            circleRadius = 5f
        ...
        val radarDataSet = RadarDataSet(radarEntries, "Temperatura en Radar").apply {
            color = Color.MAGENTA
